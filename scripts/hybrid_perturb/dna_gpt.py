...
def experiment(args):
    ...
    for idx in tqdm.tqdm(range(n_samples), desc=f"Computing {name} criterion"):
        ...
        # augmented text
        perturbed_crit = criterion_fn(sampler, perturbed_text)
        # result
        results.append({
    ...
    save_embedding_results(args.output_file, f"{name}", args.embedding, predictions)
    
if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--output_file', type=str, default="exp_main/results/hybrid/white/dna_gpt/xsum_gpt2-xl")
